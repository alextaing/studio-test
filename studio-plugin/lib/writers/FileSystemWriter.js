...
export class FileSystemWriter {
    orchestrator;
    project;
    constructor(orchestrator, project) {
     ...
     */
    writeToPageFile(pageName, pageState) {
     ...
     */
    writeToModuleFile(moduleMetadata, moduleDependencies) {
    ...
    writeToSiteSettings(siteSettingsValues) {
     ...
     */
    syncFileMetadata(updatedUUIDToFileMetadata) {
        ...
        const modulesToUpdate = new Set(Object.keys(updatedUUIDToFileMetadata).filter((metadataUUID) => {
            const updatedMetadata = updatedUUIDToFileMetadata[metadataUUID];
            const isModule = updatedMetadata.kind === FileMetadataKind.Module;
            return (isModule &&
                !areEqualFileMetadata(updatedMetadata, UUIDToFileMetadata[metadataUUID]));
        }));
        for (const moduleMetadataUUID of modulesToUpdate) {
            const getModuleMetadata = (metadataUUID) => {
