...
export default async function createStudioPlugin(args) {
    const cliArgs = JSON.parse(process.env[STUDIO_PROCESS_ARGS_OBJ]);
    ...
    const localDataMappingManager = studioConfig.isPagesJSRepo
        ...
        : undefined;
    const orchestrator = new ParsingOrchestrator(tsMorphProject, studioConfig, localDataMappingManager?.getMapping);
    ...
    return {
        ...
        config(config) {
            const studioViteOptions = getStudioViteOptions(args, studioConfig, pathToUserProjectRoot);
        ...
        configureServer: createConfigureStudioServer(fileSystemManager, gitWrapper, orchestrator, localDataMappingManager, pathToUserProjectRoot, studioConfig.paths),
    };
}
function getProjectRoot(cliArgs) {
    if (!cliArgs.root) {
