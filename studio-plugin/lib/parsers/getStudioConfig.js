 ...
 */
export default async function getStudioConfig(pathToProjectRoot, cliArgs = {}) {
    try {
...
async function getStudioConfigInternal(pathToProjectRoot, cliArgs) {
    const defaultConfig = {
        isPagesJSRepo: false,
    ...
    const mergedConfig = lodash.merge({}, defaultConfig, studioConfig, {
        port: cliArgs.port,
    });
    return mergedConfig;
}
 ...
 */
function normalizePaths(config, pathToProjectRoot) {
    ...
    Object.keys(updatedConfig.paths).forEach((pathType) => {
        ...
        updatedConfig.paths[pathType] = upath.normalize(updatedConfig.paths[pathType]);
    });
 ...
 * @throws {@link ParsingError|FileIOError}
 */
async function importExistingConfig(absFilepath) {
    let importedFile;
    try {
        importedFile = await dynamicImport(absFilepath);
    }
    catch (err) {
        const importError = new FileIOError(IOErrorKind.FailedToImportFile, `Failed to import module at ${absFilepath}`, err instanceof Error ? err.stack : undefined);
        throw importError;
    }
