...
export const startPagesDevelopmentServer = async () => {
    const pagesServer = spawn("npx", ["pages", "dev", "--open-browser=false", "--local"], {
    ...
    pagesServer.stderr.on("data", (data) => {
        data.split("\n").forEach((line) => {
    ...
    const port = new Promise((resolve) => {
        ...
        pagesServer.stdout.on("data", (data) => {
...
//# sourceMappingURL=startPagesDevelopmentServer.js.map